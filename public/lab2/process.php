    ...
    if (isset($_FILES['file'])) {
        ...
        if (is_uploaded_file($_FILES['file']['tmp_name'])) {
            ...
            if ($file_size > $max_file_size) {
                echo "Файл перевищує допустимий розмір.";
                exit;
            }
            ...
            if (file_exists($file_path)) {
                $unique_suffix = uniqid(); // генерує унікальний ID
                $file_name = pathinfo($file_name, PATHINFO_FILENAME) . '_' . $unique_suffix . '.' . pathinfo($file_name, PATHINFO_EXTENSION);
                $file_path = $uploads_dir . '/' . $file_name;
                echo "Файл з таким ім'ям вже існує. Ім'я файлу було змінено на: $file_name<br>";
            }

            if (!is_dir($uploads_dir)) {
                if (!mkdir($uploads_dir, 0755, true)) {
                    die("Не вдалося створити директорію.");
                }
            }
            ...
            if (move_uploaded_file($file['tmp_name'], $file_path)) {
                ...
                echo "Сталась помилка при завантаженні файлу.";
            }
        } else {
